...
dumbbell_plot_with_segment <- function(data, xsegment) {
  ggplot(data, aes(x = 0, xend = change, y=ylabel, yend = ylabel)) +
  geom_dumbbell(colour="#a3c4dc", inherit.aes = TRUE, size_x = 3, size_xend = 3, colour_xend="#0e668b", 
                         dot_guide=TRUE, dot_guide_size=0.15) +
  geom_segment(aes(x=xsegment, xend=xsegment, y=1, yend=5), color="darkgrey", linetype="dashed") +
  scale_x_continuous(limits=c(-250,3000), labels=dollar_format())
}

annotate_overall_segment <- function(x) {
  annotate("text", x=x, y=2.5, label="Overall\nAverage\nChange", size=3, family = "Trebuchet MS")
}
```

```{r plot}
# To ask: 
# - How do we use data elements for x or y tick labels
# - How do I get annotations to the right of the plot (outside of it)
# - How to put axis title on one side and axis labels on the other

public <- net_price_by_income %>%
  filter(sector == 'Public') %>% 
  dumbbell_plot_with_segment(public_overall) +
  ...
  theme_he() +
  ylab("Public") +
  annotate_overall_segment(1800)

private <- net_price_by_income %>%
  filter(sector == 'Private') %>% 
  dumbbell_plot_with_segment(private_overall) +
  theme_he() +
  ylab("Private") +
  annotate_overall_segment(2700)
